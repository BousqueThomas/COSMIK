...
def ik_pipe_qp(no_sujet, task, data_path):
    ...
    # fichier_csv_lstm_mks= f"{data_path}"+subject+"/marche/LSTM/jcp_coordinates_ncameras_augmented_marche.csv"
    ...
    for seg_name, mks in seg_names_mks.items():
        viz.viewer.gui.addXYZaxis(f'world/{seg_name}', [1, 0., 0, 1.], 0.015, 0.06)
...
class IK_Quadprog:
    ...
    def solve_ik_sample(self, ii: int, meas: Dict)->None:
        ...
        if ii == 0 : # Init to be done with ipopt
            ...
            nlp = cyipopt.Problem(
                n=len(self._q0), #nombre de variable de décision 
                m=len(cl), #Nombre de contraintes ici juste une
                problem_obj=Ipopt_warm_start(self._model,meas,self._keys_to_track_list), #fonction cout defini dans ipopt_warm_start
                lb=lb, #borne inférieur dees variables de décision
                ub=ub, #borne supérieur dees variables de décision
                cl=cl, #borne inférieur des contraintes
                cu=cu, #borne supérieur des contraintes
                )

            # IPOPT options
            nlp.add_option('tol',1e-3) #tolérence de convergence
            nlp.add_option('print_level',0) #niveau de détail à imprimer

            # Solving the IPOPT problem
            q_opt, info = nlp.solve(self._q0) #q_opt est la config articulaire optimale qui minimise la fonction coût et info donne dees informations supplémentaires sur la solution (statut de convergence, nombre d'itération, ..)
...
def ik_pipe (no_sujet, task, data_path):
    ...
    subject='sujet_0'+str(no_sujet)
    fichier_csv_lstm_mks_calib = f"{data_path}"+subject+"/"+task+"/LSTM/jcp_coordinates_ncameras_augmented_"+task+"_"+str(no_sujet)+".csv"
